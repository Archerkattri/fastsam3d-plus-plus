...
def downsample_with_feature_fusion(
    ...
    fused_scores = torch.zeros(M, device=device, dtype=raw_scores.dtype)

    if fusion_mode == 'mean':
        fused_scores.index_add_(0, inverse_indices, raw_scores)
        counts = torch.zeros(M, device=device, dtype=torch.float32)
    ...
    new_coords_scores = torch.cat([
        ...
        new_coord_batch[:, 1:].float() 
    ], dim=1)

    if M > max_coords:
        perm = torch.randperm(M, device=device)[:int(max_coords)]
