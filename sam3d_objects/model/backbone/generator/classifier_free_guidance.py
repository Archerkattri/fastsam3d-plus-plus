...
class ClassifierFreeGuidance(torch.nn.Module):
    ...
    def enable_adaptive_guidance(self, gamma_bar: float = 0.94, warmup: int = 2, max_order: int = 1):
        ...
        per-step compute on aligned steps). Composes with Fast-SAM3D's existing TaylorSeer/
        HiCache module caching + carving. Native — no monkey-patching. Per-trajectory state
        is reset by FlowMatching.generate_iter via reset_adaptive_guidance()."""
    ...
    def inner_forward(self, x, t, is_cond, strength, *args_cond, **kwargs_cond):
        ...
        if adacfg is not None:
            # cache cosine (next decision) + guidance term g = v_cfg - y_cond
            # (convention-agnostic: scalar or per-modality strength).
...
class ClassifierFreeGuidance_token(ClassifierFreeGuidance):
    ...
    def inner_forward(self, x, t, is_cond, strength, *args_cond, fast_update_indices=None, **kwargs_cond):
        y_cond = self.backbone(x, t, *args_cond, fast_update_indices=fast_update_indices, **kwargs_cond)
        ...
        if is_cond:
            ...
            y_uncond = self.backbone(x, t, *args_cond, fast_update_indices=fast_update_indices, **kwargs_cond) 

            return self._cfg_step(y_cond, y_uncond, strength)

    def forward(self, x, t, *args_cond, fast_update_indices=None, **kwargs_cond):
        ...
        if len(args_cond) + len(kwargs_cond) == 0:  # unconditional
            ...
            if self.training:
                ...
                return self.backbone(x, t, *args_cond, fast_update_indices=fast_update_indices, **kwargs_cond)
            else:  # inference mode
                strength = get_strength(self.strength, self.interval, t)
                is_cond = not any(x > 0.0 for x in _pytree.tree_flatten(strength)[0])
                
                return self.inner_forward(
                    x, t, is_cond, strength, *args_cond, fast_update_indices=fast_update_indices, **kwargs_cond
                )
...
class ClassifierFreeGuidanceWithExternalUnconditionalProbability_token(ClassifierFreeGuidanceWithExternalUnconditionalProbability):
    ...
    def forward(self, x, t, *args_cond, fast_update_indices=None, p_unconditional=None, **kwargs_cond):
        # p_unconditional should be a value in [0, 1], indicating the probability of unconditional
        ...
        if len(args_cond) + len(kwargs_cond) == 0:  # unconditional
            ...
            if self.training:
                ...
                return self.inner_forward(
                    ...
                    strength, 
                    *args_cond, 
                    fast_update_indices=fast_update_indices, 
                    **kwargs_cond
                )
