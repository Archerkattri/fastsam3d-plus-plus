...
# --------------------------------------------------------------------------- #
# DMD / Prony exponential velocity forecast (tree-aware) — exponential analogue #
# of HiCache: fit a linear propagator to raw velocity-tree snapshots and advance #
# by eigenvalue powers. Exact on the exponential solution class where Hermite    #
# polynomials drift. Trees flattened to one vector per snapshot, DMD'd, unflat'd. #
# --------------------------------------------------------------------------- #
def _dmd_forecast_flat(snapshots, k, rank=0, ridge=1e-8):
    if len(snapshots) < 4:
        return snapshots[-1].clone()
    dt = snapshots[-1].dtype
    V = torch.stack(snapshots, dim=1).to(torch.float64)
    X, Xp = V[:, :-1], V[:, 1:]
    try:
...
def dmd_forecast_tree(state: Dict[str, Any]) -> Any:
    """DMD forecast of the velocity tree at the current skip step (uniform-tail >=4
    snapshots; fractional horizon lambda**(k/spacing)); Hermite fallback on warm-up."""
    ...
    if len(snaps) >= 4:
        ...
        if spacing > 0:
            ...
            if len(tail) >= 4:
                trees = [v for _, v in reversed(tail)]
                flat = [_pytree.tree_flatten(t) for t in trees]
