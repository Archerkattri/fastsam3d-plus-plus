...
from PIL import Image
import numpy as np
# Demo-UI-only deps (gradio/seaborn/matplotlib) — optional so the Inference class is
# importable in headless/bench environments without the full demo stack installed.
try:
...
try:
    ...
    # Correct real-part-first (w,x,y,z) fallbacks matching pytorch3d, so the Inference
    # class imports in envs without a pytorch3d build (e.g. Blackwell/cu128).
    ...
    def quaternion_invert(q):
        scaling = (q * q).sum(dim=-1, keepdim=True)
        return (q * q.new_tensor([1.0, -1.0, -1.0, -1.0])) / scaling
...
class Inference:
    ...
    def get_params(self,args):
        ...
        ss_params = {k: v for k, v in args_dict.items() if k.startswith("ss_")}
        slat_params = {k: v for k, v in args_dict.items() if k.startswith("slat_")}
        mesh_params = {k: v for k, v in args_dict.items() if k.startswith("mesh_")}
        enable_mesh = args.enable_mesh_aggregation

        self._pipeline.ss_params = ss_params 
        self._pipeline.slat_params = slat_params
